...
$(document).ready(function () {
    ...
    strs = $('#id-input-F00000053');
    bezn = $('#id-input-bzrname');
    strnr = $('#id-input-strnr');
    plz = $('#id-input-F00000054');
    otnrh = $('#id-input-otnr');
    hnrh = $('#id-input-hnr');
    hausnr = $('#id-input-F00000016');
    hausnrz = $('#id-input-F00000084');
    tohnr = $('#id-input-bisF00000016');
    //beznr = $('#id-input-bzrnr');
    beznrh = $('#id-input-bzrnr');
    otname = $('#id-input-otname');
    city = $('#id-input-F00000035');
    landh = $('#id-input-land');
    ...
    etrs89x = $('#id-input-etrs89_x');
    etrs89y = $('#id-input-etrs89_y');
    onlyber = $('#id-input-onlyberlin');
    iseid = $('#id-input-eid-true');
    body = $('body');
    blocklabel = $('#id-input-blocklabel');
...
if (blocklabel.length > 0){
    $.each(blocklabel.val().split(";"), function(){
        blocklabelarr.push($.trim(this));
    });
    if(blocklabelarr.indexOf(body.attr('id')) > -1){
        $("[class*='label_before']").each(function() {
            var el = $(this);
            var classList = el.attr('class').split(/\s+/);
            $.each(classList, function (index, item) {
                if (item.startsWith("label_before")){
                    el.toggleClass("label_above", true);
                    el.toggleClass("label_before", false);
                }
              })
        });
        //$('.block.narrow.label_before').addClass('block.narrow.label_above').removeClass('block.narrow.label_before');
    }
}
...
function initRBS(){
    ...
    optionhnr = {
        script: function () {return url_rbs_data + "strnr=" + $('#id-input-strnr').val() + '&otnr='+ $('#id-input-otnr').val()},
        varclass: 'house',
        timeout:10000,
        maxentries: 2500,
        minchars: 0,
...
function checkCity(){
    if (city.val() != undefined){
        if (city.val().toLowerCase() === "berlin"){// && landh.val().indexOf("000") >= 0){ // === "000|Deutschland") {
            initRBS();
            var str_xml = new bsnrbs.AutoSuggest('id-input-F00000053', optionstr);
            var hnr_xml = new bsnrbs.AutoSuggest('id-input-hnr', optionhnr);
            var hnr_xml = new bsnrbs.AutoSuggest('id-input-bisF00000016', optionhnrto);
            var plz_xml = new bsnrbs.AutoSuggest('id-input-F00000054', optionplz);
            return true;
        }else{
            //bzrnr.val('');
            beznrh.val('');
            ...
            etrs89x.val('');
            etrs89y.val('');
            bsnrbs.AutoSuggest('id-input-F00000053', optionstr);
            bsnrbs.AutoSuggest('id-input-hnr', optionhnr);
            bsnrbs.AutoSuggest('id-input-bisF00000016', optionhnrto);
            bsnrbs.AutoSuggest('id-input-F00000054', optionplz);
            return false;
        }
    }
};
...
function checkAddress(){
	if (city.val() != undefined){
		if (city.val().toLowerCase() == 'berlin'){
            //alert ('beBerlin');
			if (strnr.val().length > 0){ //&& plz.val().length > 0){ //&& hnrh.val().length > 0){// && bzrnr.val().length < 1){
        ...
        for (var i=0;i<results.length;i++)
        {
            if (results[i].nodeType == 1){
                ...
                if (postleit.length > 0){
                    plz.val(postleit);
                    beznrh.val(myres.getAttribute('bzrnr'));
                    //bzrnr.val(myres.getAttribute('bzrnr'));
                    getCoords(myres);
                    //console.log(plz.val());
                }else{
                    myres= '';
...
function checkStreet(){
	if (city.val() != undefined){
		if (city.val().toLowerCase() == 'berlin'){
			//alert ('beBerlin');
			if (strnr.val().length > 0 || strs.val().length > 0){ //|| strnr.val.length > 0){ //&& plz.val().length > 0){ //&& hnrh.val().length > 0){// && bzrnr.val().length < 1){
                var requestURL = url_rbs_strliste;
                var argnr = "strnr=" + strnr.val() + '&otnr='+ $('#id-input-otnr').val();
                var args = "strname=" +strs.val() + '&otnr='+ $('#id-input-otnr').val();
                if (meth == "GET"){
					...
					if (bezliste.nodeType == 1){
                        if (bezliste.childNodes.length >= 3){
                            if (bezliste.childNodes.item(1).nodeType == 1){
                                ...
                                var beznr_ = bezs.getAttribute('bzrnr');
                                if (bezs.childNodes.length == 5){
                                    ...
                                    //strs.val(strname); strnr.val(strnr_); objtyp.val(strtyp_ + "| " + strtyp_); bezn.val(bezn_); bzrnr.val(beznr_); beznrh.val(beznr_);  otname.val(otnam_); otnr.val(otnr_); checkAddress();     

                                    // strs.val(strname); strnr.val(strnr_); hnrh.val(''); objtyp.val(strtyp_ + "| " + strtyp_); bezn.val(bezn_); bzrnr.val(beznr_); beznrh.val(beznr_);  otname.val(otnam_); otnr.val(otnr_); checkAddress();     
                                }
                            }
...
$(document).on("focusin", ".inputcontainer", function (e) {
    if (/Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor) == true){
        $('#id-input-F00000054').attr("autocomplete","new-password");
        $('#id-input-otname').attr("autocomplete","new-password");
        $('#id-input-bzrname').attr("autocomplete","new-password");
        $('#id-input-F00000053').attr("autocomplete","new-password");
        $('#id-input-hnr').attr("autocomplete","new-password");
        $('#id-input-hnr').attr("autofill","off");
        $('#id-input-bisF00000016').attr("autocomplete","new-password");
    }else{
        $('#id-input-F00000054').attr("autocomplete","off");
        $('#id-input-otname').attr("autocomplete","off");
        $('#id-input-bzrname').attr("autocomplete","off");
        $('#id-input-F00000053').attr("autocomplete","off");
        $('#id-input-hnr').attr("autocomplete","off");
